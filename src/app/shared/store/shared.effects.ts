...
export class SharedEffects {
        ...
        tap(() => {
          if (this._alertOptions.mode === 'full') {
            animate(
              ...
              document
                .querySelector('#alert .alert__content')
      ...
      return this.actions$.pipe(
        ...
        tap((payload) => {
          ...
          if (this._backdropOptions.transition === 'move') {
            ...
            animate(`#backdrop`, { top: '0px' }, { duration: 0 }).finished.then(
              () => {
                animate(
                  `#backdrop`,
                  { opacity: [0.5, 0.8, 1] },
        ...
        tap(() => {
          ...
          if (this._backdropOptions.transition === 'move') {
            ...
            animate(
              ...
              animate(`#backdrop`, { top: `${wrapper.clientHeight * -1}px` });
  ...
  sliderPageExpandContent$ = createEffect(
    () => {
      return this.actions$.pipe(
        ...
        withLatestFrom(
          this._store.pipe(select(fromReducer.getSliderPageConfig))
        ),
        map(([action, options]) => {
  ...
  overlayShow$ = createEffect(
    () => {
      return this.actions$.pipe(
        ofType(fromActions.ActionTypes.OverlayShow),
        withLatestFrom(this._store.pipe(select(fromReducer.getOverlayConfig))),
        map(([action, options]) => {
          this._overlayOptions = options;
          return action;
        }),
        map((action: fromActions.OverlayShow) => action.payload)
      );
    },
    { dispatch: false }
  );

  overlayHide$ = createEffect(
    () => {
      return this.actions$.pipe(
        ofType(fromActions.ActionTypes.OverlayHide),
        withLatestFrom(this._store.pipe(select(fromReducer.getOverlayConfig))),
        map(([action, options]) => {
          this._overlayOptions = options;
          return action;
