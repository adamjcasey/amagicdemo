...
export class BluetoothService {
  ...
  async checkPermissions() {
    ...
    if (Capacitor.isNativePlatform() && !this.layoutConfig.noDeviceMode) {
      ...
      return new Promise((resolve, reject) => {
        if (isEnabled) {
          if (this.layoutConfig.debuggingDeviceMode) {
            this.logger('BleClient is enabled? ', isEnabled);
          }
          resolve('granted');
        }
        else {
  ...
  async stop() {
    ...
    if (Capacitor.isNativePlatform()) {
      ...
      if (this.layoutConfig.debuggingDeviceMode) {
        this.logger('stop', `Device disconnected ${this.peripheral_.device.deviceId}`);
      }
    }
  }
  ...
  async waitForDosingStart(continueDose?: boolean) {
    ...
    if (Capacitor.isNativePlatform()  && !this.layoutConfig.noDeviceMode) {
      ...
      if (this.state_ === 1) {
        ...
        if (this.layoutConfig.debuggingDeviceMode) {
          this.logger('waitForDosingStart error > state of the device is not 1.');
        }
      ...
      return new Promise((resolve, reject) => {
        const controller = setInterval(() => {
          if (this.state_ === 3) {
            clearInterval(controller);
            if (this.layoutConfig.debuggingDeviceMode) {
              this.logger('Dosing done', `Dose completed successfully. State: ${this.state_}`);
            }
            resolve(true);
          }
          ...
          if (this.state_ === 4) {
            clearInterval(controller);
            if (this.layoutConfig.debuggingDeviceMode) {
              this.logger('checkDosing error', `Unpressed action device during dosing. State: ${this.state_}`);
            }
  ...
  async onDeviceDiscovered(peripheral: any) {
    if (
      ...
      if (this.layoutConfig.debuggingDeviceMode) {
        if (peripheral.device.deviceId !== this.uuid_) {
      ...
      if (Capacitor.isNativePlatform()) {
        ...
        if (this.layoutConfig.debuggingDeviceMode) {
          this.logger('Device Connected', `Device ID: ${peripheral.device.deviceId}`);
        }
      }
        ...
        try {
          ...
          if (Capacitor.isNativePlatform()) {
            ...
            if (reading.byteLength > 0) {
              ...
              if (this.layoutConfig.debuggingDeviceMode) {
                this.logger('Data from Device', bytes.join(', '));
              }
